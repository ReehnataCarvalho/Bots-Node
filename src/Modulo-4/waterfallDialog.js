...
const bot = new builder.UniversalBot(connector, [
    (session) => {
        ...
        builder.Prompts.time(session, 'Por favor, informe a data e hora da reserva(ex.: 02 de outubro às 10h)');
    },
    (session, results) =>{
        session.dialogData.dataReserva = builder.EntityRecognizer.resolveTime([results.response]);
        builder.Prompts.number(session, 'Para quantas pessoas é a reserva?')
    }, 
    (session, results) => {
        session.dialogData.quantidadePessoas = results.response;
        builder.Prompts.text(session, 'No nome de quem é a Reserva?');
    },
    (session, results) =>{
        session.dialogData.nomeReserva = results.response;

        // Vamos exibir as iformações digitadas pelo usuário:
        session.send(`Reserva Confirmada. Segue os detalhes de Reserva:
